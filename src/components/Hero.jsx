...
// Use local hero video from assets (Hero.mp4)
const heroVideo = new URL('../assets/hero video/Hero.mp4', import.meta.url).href;
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // Attempt to play the video on mount (some browsers require a user gesture, but we'll try).
    if (videoRef.current) {
      const playPromise = videoRef.current.play();
      if (playPromise && typeof playPromise.then === 'function') {
        playPromise.then(() => setIsPlaying(true)).catch(() => {
          // autoplay blocked — keep muted state and allow user to unmute/play
  ...
  const toggleFullscreen = async (e) => {
    ...
    try {
      ...
      // Ensure the video stays playing and muted state persists
      if (videoRef.current.paused) {
        videoRef.current.play();
        setIsPlaying(true);
      }
    } catch (err) {
      // noop: browsers may reject fullscreen depending on user gesture
    }
  };
    ...
    <section id="home" className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 z-20 px-6 md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-center md:justify-between items-start text-left w-full h-full pt-28 md:pt-[12%]">
        ...
        <div className="flex flex-col items-start text-left max-w-lg lg:max-w-xl w-full">
          ...
          >
            I’m an aspiring software developer with strong skills in Python, HTML, and CSS, building web and AI-enabled applications with a focus on problem solving and learning new technologies.
          </p>
        ...
        <div 
          data-aos="zoom-in"
          data-aos-delay="600"
          className="mt-12 md:mt-2 flex flex-col items-center justify-center gap-2 self-start md:self-auto"
        >
          <div className="flex flex-col items-center gap-2">
            <div className="w-14 h-14 md:w-16 md:h-16 rounded-full border border-white/20 bg-black/20 backdrop-blur-md flex justify-center items-center group-hover:scale-105 group-hover:bg-white group-hover:border-white transition-all duration-300 shadow-xl cursor-pointer" onClick={toggleMute}>
              {isMuted ? (
              // Speaker Muted Icon
            ...
            <button
              ...
              className="w-10 h-10 mt-1 rounded-full border border-white/20 bg-black/20 flex items-center justify-center hover:bg-white hover:text-black transition-colors"
              aria-label="Toggle fullscreen video"
            >
              <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8 3H5a2 2 0 00-2 2v3m13 13h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3m13-13V5a2 2 0 00-2-2h-3" />
              </svg>
            </button>

            <span className="text-white text-[9px] md:text-[11px] font-extrabold tracking-widest uppercase opacity-60 hover:opacity-100 transition-opacity mt-1">
